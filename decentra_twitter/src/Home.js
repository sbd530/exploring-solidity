...
const Home = ({ contract }) => {
    ...
    const loadPosts = async () => {
        ...
        let posts = await Promise.all(results.map(async aPost => {
            // use hash to fetch the post's metadata stored on ipfs 
            let response = await fetch(`https://ipfs.infura.io/ipfs/${aPost.hash}`)
            const metadataPost = await response.json()
            // get authors nft profile
            const nftId = await contract.profiles(aPost.author)
